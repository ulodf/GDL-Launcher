...
class UnifiedLogFrame(ttk.Frame):
    ...
    def append(self, msg: str):
        ...
        if msg.lstrip().startswith("#") or " # F:" in msg or "\n# F:" in msg:
            return
        if not msg.endswith('\n'):
            msg += '\n'
...
class UnifiedLinksFrame(ttk.Frame):
    ...
    def search_url(self):
        ...
        # Display results
        if found_instances:
            result_text = f"Results for URL: {search_url}\n"
            result_text += "------------------------------------------\n"
            result_text += "Found in instances:\n"
            for idx, exact_match in found_instances.items():
                match_type = "Exact match" if exact_match else "Partial match"
                result_text += f"Instance {idx+1}: {match_type}\n"
            self._update_results_box(result_text)
        else:
            self._update_results_box(f"URL '{search_url}' not found in any instance")
    ...
    def check_bulk_urls(self):
        ...
        # Display results
        result_text = "Bulk URL Check Results\n"
        result_text += "=======================\n"
        ...
        for url, found_instances in results.items():
            ...
            if found_instances:
                ...
                result_text += "  Not found in any instance\n"
    ...
    def filter_by_domain(self):
        ...
        # Display results
        if filtered_urls:
            result_text = f"URLs with domain '{domain_filter}':\n"
            result_text += "=======================\n"
            
            for idx, urls in filtered_urls.items():
                result_text += f"Instance {idx+1}:\n"
                for url in urls:
                    result_text += f"  {url}\n"
                result_text += "------------------------------------------\n"
            ...
            self._update_results_box(f"No URLs found with domain '{domain_filter}'")
    ...
    def extract_base_url(self, url: str) -> str:
        ...
        try:
            ...
            if len(parts) >= 3:  # has at least "http:/domain.com"
                domain = parts[2]

                # For certain sites, also include the user/profile path
    ...
    def quick_add_url(self):
        ...
        if found_instances:
            result_text = "URL already exists in:\n"
            for idx, exact_match in found_instances.items():
                match_type = "Exact match" if exact_match else "Partial match"
                result_text += f"Instance {idx+1}: {match_type}\n"
        ...
        # Show confirmation
        messagebox.showinfo("URL Added", f"Added URL to Instance {instance_idx + 1}")
    ...
    def refresh(self):
        ...
        for inst in self.get_instances():
            ...
            if not links:
                continue
            self.box.insert("end", f"Instance {inst.idx + 1}:\n")
            for ln in links:
                self.box.insert("end", f"  {ln}\n")
            self.box.insert("end", "\n")
        self.box.config(state="disabled")
    ...
    def export_links(self):
        ...
        if filename:
            try:
                ...
                messagebox.showerror("Error", f"Failed to export links: {e}")
    ...
    def export_with_mapping(self):
        ...
        if filename:
            try:
                ...
                messagebox.showinfo("Success", f"URL mappings exported to {filename}")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to export URL mappings: {e}")
    ...
    def import_links(self):
        ...
        try:
            ...
            messagebox.showerror("Import Error", f"Failed to import links: {e}")
    ...
    def _import_json_links(self, filename):
        ...
        # Show results
        result_msg = "Import Results:\n"
        for idx, count in imported_counts.items():
            if count > 0:
                result_msg += f"Instance {idx+1}: {count} URLs added\n"
...
class InstanceFrame(ttk.Frame):
    def __init__(self, master: ttk.Notebook, idx: int, get_global_opts, log_callback):
        ...
        self.links_file = DATA_DIR / f"links_instance_{idx + 1}.txt"
        ...
        self.archive_var = tk.StringVar(value=str(DATA_DIR / f"archive_instance_{idx + 1}.txt"))
    ...
    def _save_settings(self):
        ...
        settings_file = DATA_DIR / f"settings_instance_{self.idx + 1}.json"
        try:
            with open(settings_file, 'w', encoding='utf-8') as f:
                json.dump(settings, f, indent=2)
        except Exception as e:
            self.log_callback(f"[Inst {self.idx + 1}] Error saving settings: {e}\n")
    ...
    def _load_settings(self):
        ...
        try:
            ...
            self.log_callback(f"[Inst {self.idx + 1}] Error loading settings: {e}\n")
    ...
    def _set_tab_status(self, running: bool):
        ...
        self.master.tab(self, text=f"Instance {self.idx + 1} {symbol}")
        if running:
    ...
    def _enqueue_output(self, pipe):
        ...
        try:
            ...
            self._append_log("Reading gallery-dl output...\n")
    ...
    def stop(self):
        ...
        try:
            self._append_log("Stopping download process...\n")
            self.log_callback(f"[Inst {self.idx + 1}] Stopping process\n")
            ...
            if self.is_running():
                self._append_log("Process not responding, force killing...\n")
                if platform.system() == "Windows":
    ...
    def _save_links(self):
        ...
        try:
            ...
            self.log_callback(f"[Inst {self.idx + 1}] Links saved to {self.links_file}\n")
        except OSError as e:
            messagebox.showwarning("Save failed", f"Could not write {self.links_file}: {e}")
    ...
    def _load_links(self):
        ...
        if file_path:
            try:
                ...
                self.log_callback(f"[Inst {self.idx + 1}] Links loaded from {file_path}\n")
            except Exception as e:
                messagebox.showerror("Load failed", f"Could not read {file_path}: {e}")
    ...
    def _save_log(self):
        ...
        filename = filedialog.asksaveasfilename(
            ...
            filetypes=[("Log files", "*.log"), ("Text files", "*.txt"), ("All files", "*.*")],
            initialdir=DATA_DIR,
            title=f"Save Instance {self.idx + 1} Log"
        )
        if filename:
            try:
                with open(filename, 'w', encoding='utf-8') as f:
    ...
    def _start_normal_download(self, links):
        ...
        archive_file = self.archive_var.get()
        if archive_file:
            if "archive_instance_" in archive_file and not archive_file.endswith(f"_{self.idx + 1}.txt"):
                archive_file = str(DATA_DIR / f"archive_instance_{self.idx + 1}.txt")
                self.archive_var.set(archive_file)
                self.log_callback(f"[Inst {self.idx + 1}] Using unique archive file: {archive_file}\n")
        ...
        self.log_callback(f"[Inst {self.idx + 1}] Starting gallery-dl: {cmd_str}\n")
        self._append_log(f"Starting download with all settings\n{cmd_str}")
        ...
        try:
            ...
            self._append_log(f"Process started with PID {self.proc.pid}\n")
            ...
            self.log_callback(f"[Inst {self.idx + 1}] Failed to start: {e}\n")
            messagebox.showerror("Start failed", f"Failed to start gallery-dl: {e}")
...
class DownloadHistoryFrame(ttk.Frame):
    ...
    def refresh_history(self):
        ...
        try:
            ...
            messagebox.showerror("Database Error", f"Error refreshing history: {e}")
    ...
    def clear_history(self):
        ...
        if messagebox.askyesno(
            ...
            try:
                ...
                messagebox.showerror("Database Error", f"Error clearing history: {e}")
    ...
    def add_download_entry(self, instance_id, url, file_type, status="Success"):
        ...
        try:
            ...
            print(f"Error adding download to history: {e}")
    ...
    def export_history(self):
        ...
        try:
            ...
            with open(filename, 'w', newline='') as csvfile:
                ...
                for row in rows:
                    timestamp, instance_id, url, file_type, status = row
                    writer.writerow([timestamp, f"Instance {instance_id + 1}", url, file_type or "Unknown", status])

            conn.close()
            messagebox.showinfo("Success", f"Download history exported to {filename}")
        except Exception as e:
    ...
    def run(self):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                print(f"Browser extension server started on port {self.port}")
...
class Application(tk.Tk):
    def __init__(self):
        ...
        # Log startup
        self.log_frame.append("Gallery-DL Launcher started\n")
    ...
    def on_close(self):
        ...
        if running_count > 0:
            if not messagebox.askyesno(
                    "Confirm Exit",
                    f"There are {running_count} running instances. Do you want to exit anyway?"
            ):
                return
    ...
    def add_instance(self):
        ...
        self.log_frame.append(f"Added Instance {idx + 1}\n")
    ...
    def save_all(self):
        ...
        self.log_frame.append("All settings saved\n")
    ...
    def check_version(self):
        ...
        try:
            ...
            self.log_frame.append(f"Gallery-DL version: {version_text}\n")

            # Check if we need to display a hint about command-line options
            if version_text:
                self.log_frame.append("Hint: For a list of command-line options, run: gallery-dl --help\n")

        except Exception as e:
            error_msg = f"Error checking gallery-dl version: {e}"
            messagebox.showerror("Error", error_msg)
            self.log_frame.append(f"{error_msg}\n")

            # Check if gallery-dl is installed
            if isinstance(e, FileNotFoundError):
                self.log_frame.append("Gallery-dl does not appear to be installed or is not in your PATH.\n")
                self.log_frame.append("Visit https://github.com/mikf/gallery-dl for installation instructions.\n")
                messagebox.showerror("Gallery-DL Not Found",
                                    "Gallery-dl does not appear to be installed or is not in your PATH.\n\n"
                                    "Please install gallery-dl first:\n"
                                    "pip install gallery-dl\n\n"
                                    "Visit https://github.com/mikf/gallery-dl for more information.")
    ...
    def open_data_dir(self):
        ...
        try:
            ...
            self.log_frame.append(f"Opened data directory: {DATA_DIR}\n")
        except Exception as e:
            self.log_frame.append(f"Error opening data directory: {e}\n")
    ...
    def open_docs(self):
        ...
        url = "https://github.com/mikf/gallery-dl/blob/master/README.rst"
        webbrowser.open(url)
        self.log_frame.append(f"Opened documentation: {url}\n")
